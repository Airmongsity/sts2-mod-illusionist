...
/// <summary>
/// 千面 (Myriad Faces) — 1 cost Skill, Uncommon (upgraded: gains Innate, stays 1 cost). Choose a card in your hand; 幻化
/// every OTHER card in your hand into a copy of it until end of turn, then add that many 熄灭油灯
/// (Extinguished Lamp) to your draw pile. The controllable burst enabler: pick 幻爆 (Phantom Blast) and
/// your whole hand becomes free copies of it to dump in one turn — and with FluxweaveIllusionist each
...
public sealed class MyriadFacesIllusionist : CardModel
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        // Drawback: add one 熄灭油灯 (Extinguished Lamp) to the draw pile per card reshaped.
        if (transformed > 0)
        {
            await CardPileCmd.AddToCombatAndPreview<ExtinguishedLampIllusionist>(owner.Creature, PileType.Draw, transformed, owner, CardPilePosition.Random);
        }
    }

    protected override void OnUpgrade()
    {
        // Innate so this combo-enabler is online from turn 1 (cost stays 1).
